...
struct PlayerInfo {
    ...
    /// Куда и как быстро он идёт. Оттуда же, откуда её берёт достройка движения
    /// между снимками, — своего источника у неё нет.
...
struct VehicleInfo {
    ...
    /// Насколько вывернут руль, от минус единицы до единицы.
    ///
    /// Это ввод водителя, а не угол колёс в градусах: по сети едет именно он —
    /// у alt:V `steeringAngle` тех же долей. Знак у органа управления и у натива
    /// игры разный, и перевод живёт у клиента, а не здесь.
    float steer = 0.0F;
...
///
/// Состав — то из `IServerConfig` alt:V, что у нас есть на самом деле. Того,
/// чего нет, здесь нет и в помине: пустая строка вместо адреса сайта выглядела
/// бы как «сайта нет», а его просто никто не спрашивал.
struct ServerConfigInfo {
    std::string name;

    std::uint16_t port = 0;

    /// Сколько игроков сервер пускает и сколько их сейчас.
    std::size_t maxPlayers = 0;

    /// Есть ли пароль. Сам пароль отсюда не отдаётся никогда: режим, положивший
    /// его в свой журнал или отправивший в своё окно, раздал бы его игрокам.
    bool passworded = false;

    /// Сколько тактов в секунду и как далеко раздаётся мир.
    std::uint16_t tickRate = 0;
    float streamDistance = 0.0F;

    /// Подробный ли журнал. У alt:V это `debug`.
    bool verbose = false;

    /// Какие ресурсы названы к запуску. Порядок тот же, что в файле настроек.
...
struct BlipInfo {
    ...
    /// Видна ли метка всем. У alt:V это довод конструктора, а не следствие
    /// пустого списка получателей, и разница здесь не отвлечённая: метка,
    /// заведённая с `global = false` и ещё не назвавшая получателей,
    /// предназначена **никому**, а не всем. Прежде она выводилась из пустоты
    /// списка — и такая метка успевала мигнуть у всех, а не назвавшая
    /// получателей ни разу так и оставалась общей.
    bool global = true;

    /// Кому метка видна, если она не общая. У общей не спрашивается вовсе.
    ///
...
public:
    ...
    /// Назначает машине ведущего — того, кто считает её физику.
    ///
    /// Обычно ведущего выбирает сервер: за рулём — водитель, без водителя —
    /// ближайший. Этим выбор можно перебить: например чтобы машина, за которой
    /// следит камера режиссёрского режима, считалась у зрителя, а не у того, кто
    /// случайно оказался ближе.
    ///
    /// `sticky` — держать ли назначенного и дальше. Без него назначение
    /// одноразовое: ведущий встанет сейчас, а со следующей раздачи выбор пойдёт
    /// обычным порядком. У alt:V это `disableMigration`, и смысл тот же.
    ///
    /// false — машины уже нет либо названного игрока нет на сервере. Молчать
    /// здесь нельзя: назначение, не сделанное молча, выглядит как сделанное.
    virtual bool setVehicleOwner(shared::VehicleId id, shared::PlayerId owner, bool sticky) = 0;

    /// Возвращает выбор ведущего серверу. false — машины уже нет.
    virtual bool clearVehicleOwner(shared::VehicleId id) = 0;
    ...
    /// Чем сервер объявил себя при запуске.
    ///
    /// Читается, но не правится: настройки лежат в файле рядом с сервером и
    /// принадлежат хозяину сервера, а не режиму. Правь их скрипт — прочитанное
    /// им и лежащее в файле разошлись бы, и после перезапуска сервер вёл бы
    /// себя иначе, чем до него.
    [[nodiscard]] virtual ServerConfigInfo config() const = 0;

    /// Что этому игроку сейчас раздаётся.
    ///
    /// Род и номер на каждую сущность. Спрашивается у сервера, а не у клиента:
    /// решает, кому что отдавать, сервер, и его ответ и есть правда — клиент
    /// узнаёт о ней позже и может ещё не успеть.
    ///
    /// Игроки сюда не попадают: их сервер раздаёт всем, кто в дальности, но
    /// списка «кому какого игрока отдали» не ведёт — снимки идут связкой, а не
    /// по одному. Сказать об этом честнее, чем выдумать список.
    [[nodiscard]] virtual std::vector<std::pair<shared::EntityKind, std::uint32_t>>
    streamedTo(shared::PlayerId id) const = 0;

    /// Поднять, остановить или перезапустить ресурс по имени.
    ///
    /// **Исполняется не сейчас, а перед ближайшим тактом**, и это не придирка:
    /// просьба приходит изнутри изолята одного из ресурсов, а остановка
    /// разбирает изолят целиком. Останови мы ресурс здесь и сейчас — разобрали
    /// бы стек, по которому сюда пришли, и сервер упал бы вместе с ним.
    /// Остановивший сам себя тем и опасен, что это как раз то, чего просит
    /// панель управления.
    ///
    /// Ответ поэтому означает «просьба принята», а не «сделано»: сделать это
