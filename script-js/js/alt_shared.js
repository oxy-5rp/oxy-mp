...
(function build(alt) {
    ...
    /// Начатые замеры времени, по имени.
    const timers = new Map();

    /// Куда писать строку из общей части.
    ///
    /// Разрешается при вызове, а не при сборке: общая часть исполняется первой,
    /// и ни серверного, ни клиентского слоя в это мгновение ещё нет. Взяв `log`
    /// заранее, мы взяли бы пустоту — и замер времени молчал бы навсегда.
    function say(line) {
        const write = alt.server?.log ?? alt.client?.log;
    ...
    const shared = {
        ...
        /// Отпечаток строки, как его считает alt:V.
        ///
        /// Через `crypto` из Node, а не своим счётом: SHA-256 написан и выверен
        /// задолго до нас, а свой пришлось бы сопровождать вечно — и первая же
        /// ошибка в нём проявилась бы не тут, а там, где отпечатком что-нибудь
        /// сверяют.
        ///
        /// Строчными, как у alt:V: сравнивают их обычно строкой, и разошедшийся
        /// регистр не совпал бы ни разу.
        stringToSHA256(text) {
        ...
        /// Замер времени по имени — тот же приём, что у `console.time`.
        ///
        /// Свой, а не проброс в `console`: у alt:V это его собственный вывод, с
        /// его же приставкой ресурса. Ушедшее в `console` потерялось бы среди
        /// строк Node и не легло бы в журнал сервера.
        time(name) {
            timers.set(String(name), Date.now());
        },
        ...
        timeEnd(name) {
            ...
            if (started === undefined) {
                // Молчать нельзя: тот, кто перепутал имя, решил бы, что замер
                // прошёл и ничего не занял.
                say(`timeEnd: замера «${key}» не начинали`);
                return;
            }

            timers.delete(key);
            say(`${key}: ${Date.now() - started} ms`);
        },
