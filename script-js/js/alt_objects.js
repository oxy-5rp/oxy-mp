...
(function build(alt) {
    ...
    class BaseObject {
        ...
        constructor(type) {
            ...
            // порознь: заведись новый род — событие достанется ему само. У
            // alt:V оно объявляется на всякий объект слоя, и обработчик,
            // ведущий их учёт, обязан увидеть их все.
            //
            // Объявляется после того, как объект попал в перечень: обработчик
            // вправе спросить `BaseObject.allOf` и должен найти там пришедшего.
            //
            // Готовы к этому мгновению номер и род, но не поля наследника: они
            // ставятся после `super()`. Так и у alt:V — довод там объявлен
            // `BaseObject`, а не меткой и не зоной, и читать у него цвет метки
            // не предполагается.
            server.fireLocal('baseObjectCreate', [this]);
        }
        ...
        destroy() {
            ...
            // О смерти — после уборки из перечня и после снятия признака жизни:
            // обработчик, спросивший `valid`, обязан услышать «нет», а не
            // застать объект живым в событии о его смерти.
            server.fireLocal('baseObjectRemove', [this]);
        }
    ...
    /// Объявляет вход в зону или выход из неё — обоими именами сразу.
    ///
    /// **Имя у этого события менялось, и мы знали только старое.** У нынешнего
    /// alt:V оно зовётся `entityEnterColshape` и `entityLeaveColshape`; имена
    /// `enterColshape` и `leaveColshape` остались от прежних его сборок и от
    /// RAGE MP. Мы объявляли только вторые — и режим, написанный под alt:V,
    /// не узнавал о зонах ничего: зоны считались правильно и оповещали пустоту.
    /// У проверенного режима на новые имена подписаны два места.
    ///
    /// Объявляются оба, а не одно новое, и это не нерешительность: старое имя
    /// слушают ресурсы, написанные под RAGE MP и под старый alt:V, а стоит
    /// второе объявление одного обхода списка подписчиков, которого почти
    /// всегда нет. Отнимать работающее ради чистоты здесь не за что.
    ///
    /// Порядок доводов у обоих один и тот же — зона, потом сущность, — и это
    /// порядок alt:V, а не наш выбор.
    ///
    /// Имена пишутся целиком, а не собираются из кусков. Собранное работало бы
    /// точно так же, но его не найти поиском — ни человеку, ни машинной сверке
    /// с объявлениями alt:V, по которой здесь считают паритет. Имя события —
    /// его опознавательный знак; спрятанное в шаблонной строке, оно числится
    /// отсутствующим.
    function announce(shape, entity, entered) {
        if (entered) {
            server.fireLocal('entityEnterColshape', [shape, entity]);
            server.fireLocal('enterColshape', [shape, entity]);
            return;
        }

        server.fireLocal('entityLeaveColshape', [shape, entity]);
        server.fireLocal('leaveColshape', [shape, entity]);
    }
    ...
    class Colshape extends BaseObject {
        ...
        /// Внутри ли точка. Переопределяется всяким наследником.
        ///
        /// Здесь — отказ вслух, а не «нет». Тихое «нет» — это молчащая заглушка
        /// худшего рода: заведись зона нового рода без своего ответа, она
        /// никогда никого не впустила бы и не сказала бы об этом ни словом.
        /// Ошибку такого рода ищут в самой зоне, в слоях мира, в тактах — где
        /// угодно, только не в отсутствующем переопределении.
        isPointIn(_point) {
            throw new Error(`${this.constructor.name}.isPointIn: зона этого рода ` +
                            'не умеет отвечать, внутри ли точка');
        }
        ...
        refresh(players) {
            for (const player of players) {
                ...
                if (here && !was) {
                    ...
                    announce(this, player, true);
                } else if (!here && was) {
                    this.#inside.delete(player.id);
                    announce(this, player, false);
                }
            }
