...
// --- Сверка двух правд об одном игроке ---------------------------------------
//
// Клиент видит своего игрока у игры, сервер — в своём реестре. Числа эти обязаны
// сходиться, и расхождение между ними — настоящая поломка синхронизации: ни
// исключения, ни строки в журнале, а режим, спросивший сервер, получит не то,
// что человек видит у себя на экране.
//
// Прибор этот того же рода, что и весь стенд: он не чинит, а называет. И того же
// рода, что находка про текст: один вопрос, заданный с двух сторон границы.

/// Сверяется трижды, а не единожды, и мгновения выбраны не наугад: на покое
/// сойдётся что угодно. Второй раз — после того, как стенд поменял броню, выдал
/// оружие и опустил предел; третий — после переносов в помещение и обратно.
/// Расходятся стороны как раз на переменах, а не в тишине.
for (const когда of [40000, 80000, 145000]) {
    alt.on('connectionComplete', () => setTimeout(() => сверитьсяССервером(), когда));
}

function сверитьсяССервером() {
    {
        const я = alt.Player.local;
        const natives = require('natives');
        const тело = natives.playerPedId();

        alt.emitServer('paritycheck:своиЧисла', {
            pos: { x: я.pos.x, y: я.pos.y, z: я.pos.z },
            health: natives.getEntityHealth(тело),
            armour: natives.getPedArmour(тело),
            weapon: natives.getSelectedPedWeapon(тело) >>> 0,
            // Место в машине — в нумерации alt:V, как его и отдаёт слой.
            seat: я.seat,
            vehicle: я.vehicle === null ? 0 : я.vehicle.id,
        });
    }
}
...
// --- Ствол в руках или за спиной --------------------------------------------
//
// Проверяется одно: чем различаются вынутый ствол и убранный. На этом стоит
// признак Armed в снимке, и без него убравший оружие человек стоял бы у всех
// вокруг с пистолетом наготове до конца сессии.
//
// Мерить нужно с выдержкой, а не в том же кадре: и выдача оружия, и его
// убирание — движения, и ответ до их конца остаётся прежним. Первый заход этой
// пробы читал сразу и получил «безоружен» в обоих случаях — то есть померил
// свою поспешность, а не игру.
const kUnarmed = 0xA2719263;

function стволСейчас(me) {
    const ответы = [];

    // Второй довод у IS_PED_ARMED в скриптах игры бывает от 4 до 7, и чем он
    // там служит, не написано нигде. Спрашиваем всеми — так видно, какой из них
    // отвечает про руки.
    for (let вид = 4; вид <= 7; ++вид) {
        ответы.push(`${вид}:${natives.isPedArmed(me, вид)}`);
    }

    return `оружие ${natives.getSelectedPedWeapon(me) >>> 0}, армед ${ответы.join(' ')}`;
}

function погодя(мс) {
    return new Promise((готово) => alt.setTimeout(готово, мс));
}

setTimeout(async () => {
    const me = alt.Player.local.scriptID;
    const pistol = alt.hash('weapon_pistol');

    natives.giveWeaponToPed(me, pistol, 50, false, true);
    await погодя(1200);
    say(`[ствол] вынут — ${стволСейчас(me)}`);

    // Настоящее убирание: ствол остаётся выбранным, но его не видно. Это то
    // самое состояние, в которое человек уходит клавишей, а не сменой оружия.
    natives.setPedCurrentWeaponVisible(me, false, true, false, false);
    await погодя(1200);
    say(`[ствол] спрятан — ${стволСейчас(me)}`);

    // Для сравнения — смена на кулаки. Это другое состояние, и путать их нельзя.
    natives.setCurrentPedWeapon(me, kUnarmed, true);
    await погодя(1200);
    say(`[ствол] кулаки — ${стволСейчас(me)}`);

    natives.setCurrentPedWeapon(me, pistol, true);
}, 20000);

// --- Идёт ли движение, которое велел сервер ----------------------------------
//
// Спрашиваем игру напрямую и с той же стороны границы, что и клиент, — иначе
// «движение кончилось через 400 мс» неотличимо от «мы неверно спрашиваем».
alt.onServer('паритет:движение', (dict, name) => {
    const me = alt.Player.local.scriptID;
    let раз = 0;
    ...
    const watch = alt.setInterval(() => {
        ...
        // Довод 3 — тот, которым его зовут скрипты самой игры. Спрашиваем и
        // остальными: если разойдутся, виновата не игра, а наш вопрос.
        const ответы = [];
        for (const флаг of [0, 1, 2, 3]) {
            ответы.push(`${флаг}:${natives.isEntityPlayingAnim(me, dict, name, флаг)}`);
        }

        if (раз <= 3 || раз % 5 === 0) {
            say(`[движение] ${раз * 200}мс: ${ответы.join(' ')}`);
        }
    }, 200);
});
    ...
    for (let слой = объект; слой !== null; слой = Object.getPrototypeOf(слой)) {
        for (const ключ of Object.getOwnPropertyNames(слой)) {
            ...
            try {
                ...
                // Объявленный отказ — не поломка, он и должен бросать; поломка —
                // это когда свойство читает необъявленную переменную или зовёт
                // то, чего нет.
                //
                // Различаются они родом ошибки, а не словами в ней. Прежде здесь
                // стоял перечень оборотов («в oxyMP этого ещё нет», «у игры»,
                // «нельзя»), и он устаревал с каждым новым отказом: семь честных
                // отказов игрока попали в находки только потому, что были
                // написаны другими словами. Род же не устаревает: отказ — это
                // `new Error`, а поломка — `ReferenceError` или `TypeError`.
                if (беда.name !== 'Error') {
                    бросили.push(`${ключ}: ${беда.name}: ${беда.message.slice(0, 60)}`);
                }
            }
        }
    }
...
/// Значение одной строкой: вектор и цвет иначе печатаются как [object Object].
function коротко(значение) {
    if (значение !== null && typeof значение === 'object') {
        return ['x', 'y', 'z', 'r', 'g', 'b', 'a']
            .filter((ось) => ось in значение)
            .map((ось) => `${ось}=${Number(значение[ось]).toFixed(1)}`)
            .join(' ') || '{}';
    }

    return typeof значение === 'string' ? `«${значение}»` : String(значение);
}
...
function пробаЗаписи(имя, объект) {
    ...
    for (let слой = объект; слой !== null; слой = Object.getPrototypeOf(слой)) {
        for (const ключ of Object.getOwnPropertyNames(слой)) {
            ...
            // Свойство без присваивания — не находка, а объявленное решение:
            // `valid`, `blipType`, `isGlobal` у alt:V только читаются. В нестрогом
            // коде присваивание им молча ничего не делает, и без этой отсечки
            // прибор набирал полтора десятка ложных находок на каждый десяток
            // настоящих — то есть годился только на выброс.
            if (опись.get !== undefined && опись.set === undefined) {
                continue;
            }
            ...
            // Отличить «промолчал» от «пожаловался и вернулся» отсюда нельзя, и
            // пытаться не нужно: слой пишет свои жалобы мимо `alt.logWarning`
            // нарочно — пусти он их через заменяемую функцию, режим смог бы
            // заглушить собственные жалобы слоя. Жалоба стоит своей строкой в
            // журнале клиента, рядом; так `player.stamina` попал в находки
            // первым же прогоном, хотя ведёт себя правильно — натива на
            // «поставить выносливость» у игры нет вовсе.
            проверено += 1;
            ...
            if (одинаковы(было, стало)) {
                // С числами: без них список нечитаем. «armour: было 100, ставили
                // 101, стало 100» — это обрезка по пределу, то есть правильное
                // поведение; «было 0, ставили 1, стало 0» — настоящая находка.
                молчат.push(`${ключ} (было ${коротко(было)},`
                            + ` ставили ${коротко(станет)}, стало ${коротко(стало)})`);
            }
...
alt.on('connectionComplete', () => {
    setTimeout(() => {
        ...
        // Машина берётся с телом, а не любая из списка: у бестелесной читать
        // нечего, и обход по ней проходит вхолостую — так первая проба записи и
        // отчиталась «проверено 0», то есть не проверила ничего.
        //
        // Тела же может не быть вовсе: машины раздаёт сервер по расстоянию, и
        // если рядом ни одной, стенду об этом надо сказать, а не промолчать.
        const машина = alt.Vehicle.all.find((one) => one.valid);

        if (машина === undefined) {
            say('[..] машины с телом рядом нет — обходить и пробовать нечего');
        }

        обойти('Vehicle', машина);
