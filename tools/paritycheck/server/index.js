...
function пробаЗаписи(имя, объект) {
    ...
    for (let слой = объект; слой !== null; слой = Object.getPrototypeOf(слой)) {
        for (const ключ of Object.getOwnPropertyNames(слой)) {
            ...
            // журнале клиента или сервера, рядом; так `player.stamina` попал в
            // находки первым же прогоном, хотя ведёт себя правильно — натива на
    ...
    for (let слой = объект; слой !== null; слой = Object.getPrototypeOf(слой)) {
        for (const ключ of Object.getOwnPropertyNames(слой)) {
            ...
            try {
                ...
                if (беда.name !== 'Error') {
                    бросили.push(`${ключ}: ${беда.name}: ${беда.message.slice(0, 70)}`);
                }
            }
        }
    }
...
alt.onClient('paritycheck:своиЧисла', (player, увиденное) => {
    const расхождения = [];

    const далеко = Math.hypot(player.pos.x - увиденное.pos.x,
                              player.pos.y - увиденное.pos.y,
                              player.pos.z - увиденное.pos.z);

    // Полметра — не расхождение, а движение между кадром и тактом: снимок идёт
    // тридцать раз в секунду, и за это время человек успевает шагнуть.
    if (далеко > 0.5) {
        расхождения.push(`положение разошлось на ${далеко.toFixed(2)} м`);
    }

    for (const [имя, уСервера, уКлиента] of [
        ['здоровье', player.health, увиденное.health],
        ['броня', player.armour, увиденное.armour],
        ['оружие', player.currentWeapon >>> 0, увиденное.weapon],
        ['место', player.seat, увиденное.seat],
    ]) {
        if (уСервера !== уКлиента) {
            расхождения.push(`${имя}: у сервера ${уСервера}, у клиента ${уКлиента}`);
        }
    }

    if (расхождения.length === 0) {
        alt.log('[ok] сверка сторон: числа сошлись все');
        return;
    }

    alt.log(`[!!] сверка сторон: расхождений ${расхождения.length}`);

    for (const строка of расхождения) {
        alt.log(`     ${строка}`);
    }
});
...
// --- Смена помещения --------------------------------------------------------
//
// Событие приезжает от клиента: у сервера нет ни персонажа, ни натива, чтобы
...
alt.on('playerConnect', (player) => {
    setTimeout(() => {
        const где = player.pos;

        alt.log(`[помещение] переносим внутрь оружейного магазина`);
        player.pos = new alt.Vector3(22.5, -1106.3, 29.8);

        setTimeout(() => {
            alt.log('[помещение] возвращаем обратно');
            player.pos = где;
        }, 6000);
    }, 130000);
});
