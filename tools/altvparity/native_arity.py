...
def main() -> int:
    ...
    # И обратный вопрос, без которого сверка была однобокой: чего alt:V объявляет,
    # а у нас в таблице нет вовсе.
    #
    # Молчит это иначе, чем разъехавшиеся доводы, и громче: вызов такого натива
    # падает с «is not a function» посреди чужого обработчика. Нашёл это живой
    # режим — `hudSuppressWeaponWheelResultsThisFrame` в кадровом обработчике,
    # пять тысяч ошибок за минуту.
    #
    # Имена сверяются в обе стороны с подчёркиванием и без: у alt:V подчёркивание
    # означает «имя неофициальное», и в объявлениях оно то стоит, то нет.
    свои = set(наши)
    свои |= {имя.lstrip("_") for имя in наши}
    свои |= {"_" + имя for имя in наши}

    нехватка = sorted(имя for имя in их if имя not in свои)

    if нехватка:
        print("\n" + "объявлено у alt:V, нет у нас: %d" % len(нехватка))

        for имя in нехватка[:40]:
            print("  " + имя)

        if len(нехватка) > 40:
            print("  ... и ещё %d" % (len(нехватка) - 40))

    return 1 if расхождений or нехватка else 0
