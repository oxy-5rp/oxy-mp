...
def main() -> int:
    ...
    for name, (сколько, хеш) in sorted(наши.items()):
        ...
        # Разойтись можно в две стороны, и они не равны по цене.
        #
        # Наш список **короче** — беда: недостающий довод натив прочтёт из
        # неподготовленной ячейки, и это та самая молчащая ошибка, ради которой
        # сверка написана.
        #
        # Наш список **длиннее** — не беда: лишние ячейки натив попросту не
        # читает, а режим под alt:V больше объявленного там всё равно не
        # передаст. Мешать их в одну кучу нельзя: сверка, у которой половина
        # находок безобидна, приучает не смотреть на неё вовсе.
        if сколько < у_них:
            короче += 1
            print("  КОРОЧЕ  %-38s наш %d, alt:V %d   (%s)"
                  % (name, сколько, у_них, хеш))
        else:
            длиннее += 1
            print("  длиннее %-38s наш %d, alt:V %d   (%s)"
                  % (name, сколько, у_них, хеш))

    print("\nсверено %d, короче %d, длиннее %d, не найдено в объявлениях %d"
          % (len(наши), короче, длиннее, неизвестных))
    расхождений = короче
