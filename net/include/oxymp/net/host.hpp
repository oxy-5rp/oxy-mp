...
public:
    ...
    /// Откуда соединение пришло, точками. Пусто — такого соединения нет.
    ///
    /// Спрашивается у транспорта, а не запоминается при подключении: адрес
    /// принадлежит соединению, и держать его вторую копию значило бы завести
    /// второй источник правды об одном и том же.
    [[nodiscard]] std::string addressOf(PeerId peer) const;

    /// Круговая задержка до соединения, в миллисекундах.
    ///
    /// Считает её транспорт, и считает по-настоящему — по своим подтверждениям,
    /// а не по нашим ping. Свой счёт у клиента тоже есть, но он про другое: там
    /// его видит игрок, а здесь он нужен скриптам и админ-панелям.
    ///
    /// Ноль — такого соединения нет либо задержка ещё не измерена.
    [[nodiscard]] std::uint32_t latencyOf(PeerId peer) const;
