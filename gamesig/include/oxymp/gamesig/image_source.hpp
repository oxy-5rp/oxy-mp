...
///
/// Существует ровно затем, чтобы разбор каталога не зависел от того, откуда
/// взялись байты. Исполняемый файл GTA зашифрован на диске, поэтому осмысленный
/// поиск возможен только по памяти запущенной игры — но логика разрешения при
/// этом не меняется ни на строчку.
///
/// Реализаций три, и все они разные снаружи и одинаковые для разрешения:
/// файл на диске и чужой процесс — у оффлайн-утилиты sigcheck, собственный
/// образ — у клиента, работающего изнутри игры.
class ImageSource {
public:
