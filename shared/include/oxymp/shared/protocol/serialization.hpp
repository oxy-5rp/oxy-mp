...
public:
    ...
    /// Угловая скорость — шестью байтами вместо двенадцати.
    ///
    /// По две на ось, шагом в одну тысячную радиана в секунду и с пределом в
    /// тридцать два радиана в секунду — впятеро больше того, что игра когда-либо
    /// отдаёт даже у машины, кувыркающейся под откос.
    ///
    /// Нужна получателю ровно для того же, для чего и скорость: доворачивать
    /// поворот между снимками. Тысячная радиана там ничего не решает.
    void writeAngularVelocity(const Vec3& value);

    /// Ввод водителя — одним байтом вместо четырёх.
    ///
    /// Руль, газ и тормоз приходят числом от минус единицы до единицы (руль) и
    /// от нуля до единицы (газ и тормоз). Одна запись на все три, а не две:
    /// правило у них одно, и заводить второе значило бы завести и второе место,
    /// где его можно нарушить. Лишний бит знака у газа стоит вдвое меньшей
    /// точности — одной сто двадцать седьмой вместо одной двести пятьдесят
    /// пятой, — и разглядеть это в положении педали нельзя.
    void writeControl(float value);
    ...
    /// Опустошает буфер, оставляя за собой уже занятую под него память.
    ///
    /// Заведено ради рассылки снимков: она пишет в один буфер снимки всех
    /// игроков подряд, такт за тактом. Заводя писателя заново на каждый снимок,
    /// сервер при тысяче игроков выделял бы и отпускал память двадцать тысяч раз
    /// в секунду — на ровном месте и в самом горячем своём месте.
...
public:
    ...
    [[nodiscard]] Vec3 readAngularVelocity() noexcept;
    [[nodiscard]] float readControl() noexcept;
