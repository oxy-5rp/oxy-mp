...
enum class PlayerFlag : std::uint32_t {
    ...
    /// Идёт движение, которое велел сервер.
    ///
    /// Только про него, и это не половинчатость, а всё, что можно узнать.
    /// Спросить у игры «какое движение играет сейчас» нечем: `IS_ENTITY_PLAYING_ANIM`
    /// отвечает про названное ей движение, а перечислить их не даёт ни один
    /// натив. Зато про то, которое велели мы сами, спросить можно — и этого
    /// довольно: сервер помнит, что велел, и по концу узнаёт, что оно кончилось.
    ///
    /// Отсюда `playerAnimationChange` у alt:V и оба его `currentAnimation*`.
    /// Собственные движения игры — прыжок, лазание, плавание — сюда не входят:
    /// они едут своими признаками и движениями не считаются.
    Animating = 1U << 21U,
