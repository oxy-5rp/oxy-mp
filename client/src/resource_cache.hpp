...
public:
    ...
    /// Как идут дела, одной записью.
    ///
    /// Байты и файлы вместе, потому что показываются они по-разному: сверка
    /// считается файлами — их тысячи, и каждый проверяется мгновенно, — а
    /// закачка байтами, потому что весит она гигабайты, а числом файлов её
    /// длительность не описывается никак.
    struct Report {
        std::size_t filesDone = 0;
        std::size_t filesTotal = 0;

        /// Сколько байт разобрано и сколько их всего у предложенного набора.
        std::uint64_t bytesDone = 0;
        std::uint64_t bytesTotal = 0;

        /// Скорость закачки. Ноль — считать пока не по чему.
        std::uint64_t bytesPerSecond = 0;

        /// Качаем прямо сейчас или сверяем уже лежащее.
        bool downloading = false;
    };

    using Progress = std::function<void(const Report& report)>;
