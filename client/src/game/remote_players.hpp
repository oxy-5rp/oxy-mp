...
private:
    ...
    struct Puppet {
        ...
        ///
        /// Правило вынесено (`scripted_motion.hpp`), потому что спрашивают его
        /// двое: кукла и наш собственный персонаж. Разойдись они хоть в
        /// отсрочке — и у себя движение считалось бы идущим, а у соседей уже
        /// кончившимся.
        std::optional<ScriptedMotion> scripted;
    };
    ...
    /// Нужна на завод движений куклам: правило вынесено, и заводится оно на
    /// каждую куклу отдельно — состояние у него своё.
    const NativeTable& table_;
