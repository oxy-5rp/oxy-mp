...
struct UiLayer::State {
    ...
    /// Показывать ли непрозрачную крышку загрузки.
    ///
    /// Пока true, каждый кадр закрывается чёрным в самом низу — под страницей и
    /// меню, — и этим прячется вступительный ролик игры (логотипы, полицейская
    /// заставка), который иначе виден в зазор между постановкой перехвата и
    /// первым кадром страницы. Гаснет навсегда, как только игрок вошёл в мир
    /// (`feed->ready()`): после этого крышка не нужна и закрыла бы собой живую
    /// игру. Признак `ready_` не сбрасывается, но латч надёжнее — крышке нельзя
    /// вернуться после входа в мир ни при каких будущих правках.
    bool loadingCover = true;
...
std::unique_ptr<UiLayer> UiLayer::create(UiFeed& feed, Menu::Actions actions, std::string& error) {
    ...
    state.hook = PresentHook::install(
        [&state](IDXGISwapChain* swapchain) {
            // Крышка загрузки — в самом низу кадра, до страницы, окон и меню:
            // под ней идёт вступительный ролик игры, и закрыть его нужно с
            // первого же кадра, на котором стоит перехват. Держится до входа в
            // мир и гаснет навсегда — после него крышка закрыла бы живую игру.
            if (state.loadingCover) {
                if (state.feed != nullptr && state.feed->ready()) {
                    state.loadingCover = false;
                } else {
                    state.overlay.renderer.fillOpaque(swapchain);
                }
            }
