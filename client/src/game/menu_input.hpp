...
public:
    ...
    struct Actions {
        ...
        /// Какому окну сейчас отдавать ввод.
        ///
        /// Пусто или nullptr означает «нашему меню». Нужно затем, что окон в
        /// кадре несколько: наше меню, консоль и окна ресурсов, — а перехват
        /// один, и решать, кому из них уходит нажатие, обязан не он.
        std::function<cefui::Browser*()> target;
    };
...
private:
    ...
    /// Окно, которому уходит ввод прямо сейчас.
    ///
    /// Спрашивается на каждое сообщение, а не запоминается: окно ресурса может
    /// открыться и закрыться между двумя нажатиями.
    [[nodiscard]] cefui::Browser& page() const;

    /// Наше меню. Ему ввод уходит, когда никто другой его не просит.
