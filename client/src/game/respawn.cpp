...
void Respawn::resurrect(shared::Vec3 point, float heading, int player) {
    ...
    //
    // Последние два довода — числа, и открытая база не говорит о них ничего.
    // Раньше их не было вовсе, и натив читал их из обнулённого доводами
    // NativeContext (см. его комментарий) — то есть нулями и без этой строки.
    // Явные нули меняют не поведение, а то, что оно теперь названо, а не
    // подразумевается: короче объявленного — та самая молчащая ошибка,
    // которую ловит tools/altvparity/native_arity.py у сгенерированной
    // таблицы, но не у ручных вызовов вроде этого.
    invokeNative<void>(resurrect_, point.x, point.y, point.z, heading, false, false, false, 0, 0);
