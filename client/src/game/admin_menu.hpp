...
public:
    ...
    /// Что за пункт перед игроком.
    ///
    /// Нужно странице, и только ей: рисовать раздел, переключатель и поле ввода
    /// одинаковой строкой — значит заставлять игрока узнавать их по подписи.
    /// Решает по-прежнему меню, а страница лишь показывает то, чем пункт
    /// является.
    enum class Kind {
        /// Обычное действие: нажали — сделалось.
        Action,

        /// Переход в раздел.
        Section,

        /// Переключатель: у него есть состояние, и оно видно.
        Toggle,

        /// Поле ввода: нажатие на него просит у игрока строку.
        Field,

        /// Игрок в сессии: у него два действия, а не одно.
        Player,
    };
    ...
    struct Item {
        ...
        /// Включён ли переключатель. У остальных видов не значит ничего.
        bool on = false;
    };
    ...
    /// Ставит выбор на пункт с этим номером.
    ///
    /// Так меню слушает мышь: указатель наводится на пункт, а нажимает по нему
    /// всё та же press. Стрелки и мышь поэтому не спорят между собой — они
    /// говорят с меню на одном языке.
    void select(int index);

    /// Просит у игрока название модели.
    ///
    /// Раньше это делалось только выбором пункта «выдать по названию». Теперь в
    /// разделе транспорта есть поле ввода, и нажатие по нему — та же просьба.
    void askForModel();
...
private:
    ...
    /// Ждём ли строку от игрока.
    ///
    /// О чём именно спрашиваем, здесь не хранится: спрашиваем только название
    /// модели, и спрашиваем его в том самом поле, по которому нажали, — надпись
    /// на нём и есть вопрос.
    bool asking_ = false;
