...
CustomText::~CustomText() {
    // Ни правка, ни список не снимаются, и это намеренно.
    //
    // Правку вернуть можно, но разрушается подмена вместе с клиентом — то есть
    // при закрытии игры, — а игра в эти мгновения продолжает спрашивать надписи:
    // она прощается с Social Club, показывает своё и разбирает интерфейс. Сняв
    // за собой, мы отправили бы её спрашивать у пустоты, и ответом ей был бы
    // нуль вместо строки.
    //
    // Поэтому список остаётся жить: несколько десятков байт против вылета на
    // выходе — выбор не из трудных. Свои надписи при этом остаются нашими, и это
    // верно: игра всё равно закрывается.
    (void)state_.release();
}
