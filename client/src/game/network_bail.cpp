...
void __fastcall bailDetour(std::uintptr_t first, std::uintptr_t second, std::uintptr_t third,
    ...
    // Эту дверь дёргают каждый кадр — десятки тысяч раз за игру. В release о ней
    // не сказано ни слова: игроку не о чем тут читать, а журнал забился бы ею
    // целиком. Итог всё равно виден — по одной строке при снятии перехвата.
    //
    // Разбирать доводы имеет смысл только в отладочной сборке: там их читает
    // тот, кто ищет, чем игре не угодила наша сессия. Заодно там же остаётся и
    // чтение по первому доводу — оно стоит запроса к Windows на каждую строку.
    if (spdlog::should_log(spdlog::level::debug) && (attempt <= 5 || attempt % 500 == 0)) {
        spdlog::debug("игра выходит из сессии (попытка {}): доводы {:#x} {:#x} {:#x} {:#x}",
                      attempt, first, second, third, fourth);

        // Если первый довод всё-таки указатель — покажем, что по нему лежит. Не
        // для того, чтобы на это полагаться: разыменование чужого регистра уже
        // стоило одного вылета, и потому здесь сперва спрашивают у Windows.
        if (std::array<std::int32_t, 5> values{}; peek(first, values)) {
            spdlog::debug("  по первому доводу лежит: {} {} {} {} {}", values[0], values[1],
                          values[2], values[3], values[4]);
        }
    }

    // Один вызов из каждых kRelief пропускается, и это лекарство от краха на
    // восемнадцатой минуте.
    //
    // Смертельным счётом считаются отбитые **подряд**: не исполнившись шестьдесят
    // с лишним тысяч раз, функция валит игру — падает рисование, чтением по
    // минус единице внутри драйвера. Один пропуск счёт обнуляет, и крах не
    // приходит вовсе.
    //
    // Стоило это лекарство дорого и долго не годилось: пропущенный вызов роняет
    // сессию, поднимается она сама за секунду — а игра остаётся в виде города
    // сверху с плывущими облаками, и обратно не возвращалась ничем. Ни
    // `SHUTDOWN_LOADING_SCREEN`, ни `DO_SCREEN_FADE_IN`, ни снятие скриптовых
    // камер её не снимали.
    //
    // Оказалось, что это её собственный переход между персонажами
    // (`IS_PLAYER_SWITCH_IN_PROGRESS` отвечает «да»), и снимается он своим же
    // нативом — `Screen::endPlayerSwitch`, который зовётся каждый кадр. Искали
    // раньше не то: `GET_RENDERING_CAM` про переход не знает ничего.
    //
    // Число выбрано с запасом вдвое: крах приходит около шестидесяти пяти тысяч
    // отбитых подряд, пропуск идёт вдвое чаще. Чаще — незачем: каждый пропуск
    // стоит мигания сессии.
    constexpr std::uint64_t kRelief = 30000;

    const bool holding = state->holding.load();
    const bool relief = holding && attempt % kRelief == 0;

    if (relief) {
        spdlog::debug("отпускаем один вызов выхода (номер {}), чтобы обнулить счёт отбитых",
                      attempt);
    }
...
std::unique_ptr<NetworkBail> NetworkBail::install(const EngineAddresses& addresses,
    ...
    spdlog::debug("перехват выхода из сессии поставлен: {:#x}",
...
NetworkBail::~NetworkBail() {
    ...
    spdlog::debug("игру больше не держим в сессии, попыток выйти было: {}",
...
void NetworkBail::hold(bool holding) {
    ...
    spdlog::debug("удержание в сессии: {}", holding ? "включено" : "выключено");
}
