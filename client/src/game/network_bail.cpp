...
void __fastcall bailDetour(std::uintptr_t first, std::uintptr_t second, std::uintptr_t third,
    ...
    // Пропускать один вызов раз в столько-то отбитых — соблазн, и он проверен:
    // от краха это спасает (игра прожила двадцать четыре минуты вместо
    // шестнадцати, четыре пропуска, ни одного падения), но картинку губит
    // раньше. Пропущенный вызов роняет сессию, поднимается она сама за секунду —
    // а игра остаётся в переходной камере: та уходит в небо и обратно не
    // возвращается ничем. Ни `SHUTDOWN_LOADING_SCREEN`, ни `DO_SCREEN_FADE_IN`,
    // ни снятие скриптовых камер её не снимают, и сама игра при этом отвечает,
    // что не затемнена, не грузится и камерой не распоряжается.
    //
    // То есть выбор был между «крах на шестнадцатой минуте» и «небо на пятой».
    // Второе хуже, и потому здесь по-прежнему просто отбой.
