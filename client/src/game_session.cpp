...
void GameSession::serveFiles() {
    ...
    // встала ровно ноль.
    //
    // archiveOpened — тем же условием, а не отдельной придиркой. mounted,
    // streamed и described посчитаны строчками выше, **до** обхода архивов, а
    // открытый архив кладёт свои файлы и архетипы в те же очереди тем же
    // кадром — только опустеют они не раньше следующего pump(). Карта, вся
    // раздача которой лежит в одном архиве (обычное дело), проходила бы этот
    // счёт нулями в тот самый кадр, когда архив только открылся, и расстановка
    // просилась бы ровно на кадр раньше объявленного — то есть ту же беду,
    // что и раньше, только с другой стороны.
    if (!archiveOpened && mounted == 0 && streamed == 0 && described == 0) {
...
void GameSession::advance() {
    ...
    case Stage::Playing:
        ...
        // Каждый кадр, а не по событию: перехода игра не объявляет ничем, и
        // узнать о нём можно только спросив. Стоит это одного натива, а не
        // спроси мы — игрок остался бы смотреть на город сверху до конца
        // сессии.
        if (screen_.endPlayerSwitch()) {
            spdlog::debug("оборван переход игры между персонажами");
        }
