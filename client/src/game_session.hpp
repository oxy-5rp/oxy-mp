...
private:
    ...
    /// Проигрывает движения, которые велел сыграть сервер.
    ///
    /// ped — наш собственный персонаж: своё движение играет он, чужое — кукла.
    void applyAnimations(int ped);

    /// Докладывает привязки, у которых появились оба тела.
    ///
    /// Отдельным проходом, а не в мгновение прихода: привязка связывает две
    /// сущности, и в тот миг, когда о ней сказали, у нас может не быть ни одной
    /// из них — машина не доехала, модель предмета грузится, кукла чужого игрока
    /// не появилась.
    void applyAttachments(int ped);
    ...
    /// Движение, которое ещё не удалось сыграть.
    ///
    /// Набор движений игра подгружает не мгновенно, а персонажа чужого игрока
    /// может не быть вовсе — он далеко или его модель ещё грузится. Сыграть
    /// такое движение в тот же кадр, в который оно пришло, нельзя почти
    /// никогда, и без очереди `playAnimation` не работал бы вовсе.
    struct PendingAnimation {
        shared::PlayerAnimation animation;

        /// Когда пришло. По этому времени движение и забывается: набор, который
        /// не загрузился за пять секунд, не загрузится и за пятьдесят, а
        /// движение, которое сыграют минутой позже, — уже не то движение.
        Clock::time_point since;
    };

    std::vector<PendingAnimation> pendingAnimations_;
    ...
    /// Движения, которые велел сыграть сервер.
    ///
    /// Свой, отдельно от того, что живёт у RemotePlayers, и это не оплошность:
    /// тот отыгрывает признаки снимка у кукол, а этот исполняет распоряжения
    ...
    /// Что к чему привязано. Раз в кадр докладывает то, чему раньше не хватало
    /// тела: привязка приходит один раз, а тела появляются когда придётся.
    game::Attachments attachments_;
