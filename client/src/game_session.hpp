...
private:
    ...
    /// Переносит игрока в точку, названную сервером.
    ///
    /// Отдельно от самого появления, и порядок здесь обратный ожидаемому: клиент
    /// подключается к серверу только после того, как игрок оказался в мире —
    /// до этого игра ещё грузится, и подключаться незачем. Значит, в мгновение
    /// появления сервер ещё молчит, и первое появление неизбежно происходит в
    /// запасной точке.
    ///
    /// Прежде на этом настройка `spawn` у сервера и кончалась: она доходила до
    /// клиента, но опаздывала навсегда — работала только при возрождении после
    /// смерти. Перенос делается один раз и до проявления картинки, поэтому со
    /// стороны выглядит частью загрузки, а не рывком.
    void applyServerSpawn();
    void sweepScripts();
    ...
    /// Убирает за сессией, когда она кончилась.
    ///
    /// Нарисованное — метки, маркеры, точки — и клиентские половины ресурсов.
    void forgetSessionOnLeaving();
    ...
    void applySpeeches(int ped);

    /// Исполняет присланные распоряжения о телах.
    void applyBodyOrders(int ped);
    ...
    /// кто попал.
    /// Накладывает распоряжение сервера о теле: заморозку и неуязвимость.
    ///
    /// Каждый кадр, а не при изменении: подъём из мёртвых сбрасывает признаки
    /// персонажа, и наложенные однажды они пропали бы молча.
    void applyControl(int ped, bool frozenByServer);
    ...
    /// Убран ли экран загрузки. Прилипает: убранный не возвращается.
    mutable bool loadingScreenGone_ = false;
    ...
    /// Попадание локального игрока, замеченное за кадр, — для события weaponDamage.
    ///
    /// Замечает его игровой поток в детекте урона (тот же, что шлёт доклад
    /// серверу); осушается в applyServerEvents и уходит клиентским ресурсам
    /// событием. Свидетельство, а не распоряжение: доклад серверу идёт своим
    /// путём и не зависит от этого. Один поток пишет и читает, блокировки нет.
    /// Подробности и границы (offset/bodyPart пока заглушки) — в
    /// docs/combat-anim-hooks-plan.md.
    struct LocalHit {
        std::uint8_t kind;   ///< 0 — игрок, 1 — прохожий.
        std::uint32_t id;    ///< Номер цели в сессии.
        std::uint32_t weapon;
        std::uint16_t amount;
    };

    std::vector<LocalHit> pendingHits_;

    /// Клиентские половины ресурсов, поднятые в этой сессии.
    ///
    /// Списком имён, потому что остановить их придётся поимённо, а спросить
    /// движок, что у него поднято, границе нечем. Нужно это ради
    /// переподключения: движок отказывается поднимать ресурс с уже занятым
    /// именем, и не убери мы прежние, после разрыва не поднялся бы ни один.
    std::vector<std::string> startedResources_;
    ...
    /// Перенесён ли игрок в названную сервером точку появления. Сбрасывается при
    /// выходе: следующий сервер называет свою.
    bool serverSpawnApplied_ = false;
