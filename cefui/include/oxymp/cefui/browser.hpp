...
/// Чем берут содержимое файла ресурса. false — такого файла нет.
///
...
/// Ставит способ читать файлы ресурсов страницам режима.
///
/// Отдельно от запуска CEF, и это не разделение ради разделения: Chromium
/// поднимается вместе с клиентом, а свёртки с файлами режима появляются только
/// после того, как игрок выбрал сервер и клиент у него что-то скачал. Требуй
/// запуск способ чтения сразу — его пришлось бы выдумывать пустым.
///
/// Ставит сетевой поток, читает поток ввода-вывода Chromium — отсюда замок
/// внутри. Пока не поставлен, страницы берут файлы только с диска.
