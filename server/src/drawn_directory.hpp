...
public:
    ...
    /// Забывает ушедшего игрока в списках получателей — там, где они бывают.
    ///
    /// Только у меток: маркер и точку отбирает у себя тот, кто их рисует, а не
    /// список игроков, и списка у них нет вовсе.
    ///
    /// Нужно потому, что номер игрока — наименьшее свободное место
    /// (PlayerRegistry::freeId), и ушедший освобождает его следующему
    /// вошедшему. Не забудь список ушедшего, метка, назначенная ему одному,
    /// вошедшему под тем же номером досталась бы даром — не по решению
    /// пометившего её скрипта, а по совпадению номеров.
    void forgetPlayer(shared::PlayerId player)
        requires requires(Entry entry) { entry.targets; }
    {
        for (auto& [id, entry] : entries_) {
            std::erase(entry.targets, player);
        }
    }
