...
std::unique_ptr<Server> Server::start(const Config& config, std::string& error) {
    ...
    // Справочники моделей — до ресурсов: ресурс вправе спросить о модели прямо
    // из своей точки входа, а справочник, прочитанный позже, ответил бы ему
    // «не знаю» и был бы прав только формально.
    //
    // Отсутствие файла не поломка, и жалобы на него нет: он необязателен, а
    // четыре вопроса alt:V о моделях без него отказывают вслух и говорят, чего
    // не хватает. Поломка — это файл, который есть и не читается; о такой
    // сказать нужно, но сервер она не останавливает: без справочника он делает
    // всё то же, что делал до его появления.
    if (std::string failure; !server->models_.load(config.gameDataFile, failure)) {
        spdlog::error("Game data: {}", failure);
    } else if (server->models_.loaded()) {
        spdlog::info("Game data loaded from \"{}\"", config.gameDataFile.string());
    }
...
void Server::handlePlayerAppearance(net::PeerId peer, shared::PlayerAppearance appearance) {
    ...
    // Лицо и цвета берутся из того, что сервер уже знает, а не из объявления.
    //
    // У игры на них только запись: прочесть их обратно нечем, и клиент всегда
    // объявляет их умолчанием. Приняв его как есть, сервер затирал назначенное
    // скриптом лицо своими же нулями — режим назначал внешность, она вставала, а
    // через секунду возвращалась к умолчанию. Подробности — carryUnreadable.
    if (player->appearance) {
        shared::carryUnreadable(*player->appearance, appearance);
    }
