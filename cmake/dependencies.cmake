...
# nlohmann/json — разбор и запись JSON.
#
# Понадобился сразу в двух местах, и оба чужие. Первое — `skin.bin`: оформление
# сервера приходит от него ровно в том виде, в каком его кладёт alt:V, то есть
# объектом JSON. Второе — доводы событий страницы: alt.emit шлёт их массивом
# JSON, и разобрать этот массив обязан тот, кто знает, что значит имя события.
#
# Своего разбора здесь быть не должно. Оба конца — чужие: и файл оформления, и
# страница. Разбор, написанный под то, что мы видели, спотыкается на первом же
# значении, которого мы не видели, — а увидим мы его у чужого сервера и уже у
# игрока.
#
# Только заголовки: тянуть их сборку ради одного файла незачем.
...
# toml++ — чтение и запись файла настроек.
#
# Формат взят у alt:V вплоть до имён полей: игрок, знающий altv.toml, узнаёт
# oxymp.toml без объяснений, а перенести настройки с одного на другой можно
# правкой одной строки. Выдумывать свой формат ради того же самого незачем.
#
# Разбор чужой, а не свой, и это не лень. TOML только выглядит простым: строки в
# трёх видах, массивы, вложенные таблицы, даты. Свой разбор покрывает то, что
# написали мы сами, и спотыкается на первом же файле, который игрок поправил
# руками, — а поправит он его непременно.
#
# Библиотека заголовочная и собирается вместе с потребителем, поэтому подходит
# обеим сборкам разом: и клиенту со статической библиотекой времени выполнения,
# и серверу с динамической.
