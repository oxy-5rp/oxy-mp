...
void printUsage() {
    std::cerr << "Использование:\n"
                 ...
                 "  --jitter  на сколько миллисекунд рвать собственный шаг бота. Сеть на\n"
                 "            петле ровная до неправдоподобия, и без этого не проверить,\n"
                 "            выдерживает ли сглаживание неровную доставку.\n";
}
...
struct Gait {
    ...
    /// Наибольший разрыв, застигнутый на ленте, в метрах, и доля замеров,
    /// пришедшихся на достраивание вперёд.
    ///
    /// Оба нужны, чтобы не гадать, откуда берётся неровность. Скорость гашения
    /// разрыва тут ни при чём — сколько его ни размазывай, за промежуток между
    /// снимками сгладить надо ровно его, — а вот сам разрыв и есть причина.
    /// Достраивание же говорит о другом: запаса не хватило, и показывать было
    /// нечем.
    double seam = 0.0;
    double aheadSamples = 0.0;
...
int main(int argc, char** argv) {
    ...
    while (!g_stopRequested.load()) {
        ...
        for (Bot& bot : herd) {
            ...
            // Где бот находится в это самое мгновение, а не в начале оборота.
            //
            // Считалось это прежде один раз на всю стаю, до обхода, а уходило
            // положение отсюда — то есть отметку времени снимок получал позже
            // самого положения, и тем позже, чем дольше шёл оборот. С дрожанием
            // в сорок миллисекунд это четверть метра расхождения между «где» и
            // «когда», случайной величины на каждом снимке. Стенд врал ровно
            // так, как настоящий клиент не врёт никогда: у того положение
            // берётся из кадра игры, а отметка ставится мгновением позже.
            //
            // Замер неровности от этого показывал двойку там, где её нет, и
            // всякая правка расчёта оценивалась по чужому шуму.
            const float elapsed =
            ...
            // Обслуживание — последним, а не первым, и это часть того же
            // исправления. Отправляет снимок именно оно, и стоя первым, оно
            // отправляло положение, посчитанное оборотом раньше: снимок уходил
            // с отметкой этого мгновения и с положением из прошлого.
        ...
        if (now - lastReport >= std::chrono::seconds{2}) {
            ...
            if (bots == 1) {
                ...
                if (gait.samples > 0.0) {
                    ...
                    spdlog::info("  разрыв на ленте: {:.3f} м наибольший, "
                                 "достраивания {:.1f}%",
                                 gait.seam, gait.aheadSamples / gait.samples * 100.0);
