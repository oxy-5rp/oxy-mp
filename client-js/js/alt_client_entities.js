...
(function build(alt) {
    ...
    /// Что мы заказали заморозить. Ключ — дескриптор тела.
    ///
    /// Помнится здесь потому, что у игры об этом не спросить: натив только
    /// приказывает. Тела переиспользуются, и запись живёт до конца сессии — на
    /// точность это не влияет: заново заведённому телу заморозку закажут заново.
    const замороженные = new Map();
    ...
    class Entity extends WorldObject {
        ...
        /// Заморожена ли сущность.
        ///
        /// Спросить у игры об этом нечем: натива-вопроса нет, есть только
        /// натив-распоряжение. Поэтому отвечается **последнее заказанное**, а не
        /// увиденное, и разойтись они могут: `RESURRECT_PED` сбрасывает признаки
        /// персонажа, и после возрождения игра разморозит его сама, а память
        /// останется прежней.
        ///
        /// Бросать здесь нельзя, и это выяснил живой чужой режим: у alt:V
        /// `frozen` объявлен и на чтение, и на запись, а у нас не было сеттера
        /// вовсе — присваивание падало, и вместе с ним обрывалась вся настройка
        /// клиента, шедшая следом за той строкой.
        get frozen() {
            return замороженные.get(this.scriptID) === true;
        }

        set frozen(value) {
            const хотим = Boolean(value);

            замороженные.set(this.scriptID, хотим);

            if (this.valid) {
                natives.freezeEntityPosition(this.scriptID, хотим);
            }
        }
