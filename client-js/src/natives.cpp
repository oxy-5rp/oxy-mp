...
void callNative(const v8::FunctionCallbackInfo<v8::Value>& info) {
    ...
    // вернул сам натив, затем всё, что он записал по указателям.
    //
    // **Место возврата занято всегда, даже у натива, который ничего не
    // возвращает.** Прежде такой натив своего места не занимал, и список
    // выходил на единицу короче альтвшного — то есть у ста восьмидесяти двух
    // нативов доводы разъезжались со своими местами. Ошибки при этом не было ни
    // одной: список законный, числа в нём настоящие, только не те.
    //
    // Замечено живой игрой: `getActualScreenResolution` ответил «1080 на
    // undefined» вместо «1920 на 1080» — ширина уехала на место возврата,
    // высота на место ширины, а на её месте не осталось ничего. Разрешение
    // экрана тем и удобно, что неверный ответ виден сразу; у остальных ста
    // восьмидесяти одного он выглядел бы правдоподобно.
    //
    // У `void` в это место кладётся `undefined` — так его объявляет и alt:V
    // (`[void, number, number]`).
    const v8::Local<v8::Array> list =
        v8::Array::New(isolate, static_cast<int>(outgoing.size() + 1U));

    std::uint32_t at = 0;

    (void)list->Set(context, at++, gives == 'n' ? v8::Local<v8::Value>{v8::Undefined(isolate)}
                                                : answer);
