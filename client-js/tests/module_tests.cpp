...
// значению handler — а один и тот же обработчик, подписанный дважды (разом
// через on и once), путал их: снималась первая попавшаяся, и ей оказывалась
// постоянная, а не сама once-запись. Разница проявляется не сразу: после
// первого события сработают оба, как при верном поведении, так и при
// поломанном. Поломанное снимает on вместо once, once расстреливает себя
// вторым событием, и третьему звать уже некого; верное снимает ровно once и
// оставляет on стоять, и счёт растёт дальше.
TEST_CASE("a permanent listener survives a once with the same handler", "[client][js]") {
    REQUIRE(run("once-vs-on", "const alt = require('alt-client');\n"
                              "let счёт = 0;\n"
                              "const обработчик = () => alt.log(`раз ${++счёт}`);\n"
                              "alt.on('своё', обработчик);\n"
                              "alt.once('своё', обработчик);\n"
                              "alt.emit('своё');\n"
                              "alt.emit('своё');\n"
                              "alt.emit('своё');\n"));
